...
def compare():
    ...
    for model_name in models:
        ...
        try:
            ...
            log_lines.append(f"  Output: {model_name}.xlsx\n")
        except Exception as e:
            log_lines.append(f"  ERROR: {str(e)}\n")
            results[model_name] = {"error": str(e), "summary": {}, "detailed": []}

    log_lines.append(f"Run completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    return jsonify({"results": results, "run_id": timestamp})
