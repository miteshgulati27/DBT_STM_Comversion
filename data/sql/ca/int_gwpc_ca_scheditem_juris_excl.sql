...
WITH pcx_ca7jrschedexclitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7jrschedexclitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7jurisdictionexcl') }}
),
...
INT_CA_SI_JURISDICTION_EXCLSI AS (
...
SELECT
    ...
    CASE
        WHEN parent.Jurisdiction IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'Jurisdiction'
            || '-'
            || CAST(parent.Jurisdiction AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'JurisdictionExclSI' AS CVRBL_TYPE_KEY,
    ...
    CASE
        WHEN src.ScheduleExclusion IS NULL THEN NULL
        ELSE 'GWPC' || '-' || CAST(polper.PeriodID AS VARCHAR) || '-' || CAST(src.ScheduleExclusion AS VARCHAR)
    END AS CA_EXCL_KEY,
    ...
    'JurisdictionExclSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7jrschedexclitem src

INNER JOIN parent
    ON parent.FixedID = src.ScheduleExclusion
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7jrschedexclitem srcx
...
INT_CA_SI_JURISDICTION_EXCLSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_JURISDICTION_EXCLSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_JURISDICTION_EXCLSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
