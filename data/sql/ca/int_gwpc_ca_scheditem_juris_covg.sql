...
WITH pcx_ca7jrschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7jrschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7jurisdictioncov') }}
),
...
INT_CA_SI_JURISDICTION_COVGSI AS (
...
SELECT
    ...
    'GWPC' || '-' || 'JurisdictionCovgSI' AS CVRBL_TYPE_KEY,
    ...
    'JurisdictionCovgSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7jrschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7jrschedcovitem srcx
...
INT_CA_SI_JURISDICTION_COVGSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_JURISDICTION_COVGSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_JURISDICTION_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
