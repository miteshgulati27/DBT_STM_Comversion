...
WITH pcx_ca7privatepassenger AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7privatepassenger') }}
),
...
has_effective AS (
    ...
    FROM pcx_ca7privatepassenger srcx
...
INT_CA_VEH AS (
    SELECT DISTINCT
        'GWPC' || '-' || CAST(polper.PERIODID AS VARCHAR) || '-' || 'PrivatePassenger' || '-' || CAST(capp.FIXEDID AS VARCHAR)
            AS CA_CVRBL_KEY,

        'GWPC' || '-' || CAST(polper.PERIODID AS VARCHAR)
            AS POL_KEY,

        'GWPC' || '-' || CAST(polper.PERIODID AS VARCHAR) || '-' || CAST(capp.COMMAUTOLINE AS VARCHAR)
            AS POL_LINE_KEY,

        NULL AS OWNING_CVRBL_KEY,

        'GWPC' || '-' || 'PrivatePassenger'
            AS CVRBL_TYPE_KEY,
        ...
        CAST(COALESCE(capp.EXPIRATIONDATE, polper.PERIODEND) AS DATE) AS END_EXP_DT,

        'GWPC' AS SOURCE_SYSTEM,

        'PrivatePassenger' AS CVRBL_TYPE_CD,

        capp.CA7VIN AS VEH_ID_NO,

        capp.CA7MAKE AS MAKE_NAME,

        capp.CA7MODEL AS MODEL_NAME,

        capp.CA7YEAR AS MODEL_YR,

        capp.CA7VEHICLENUMBER AS VEH_NO,

        capp.CA7ORIGINALCOSTNEW AS ORIG_COST_NEW_AMT,

        capp.CA7STATEDAMOUNT AS STATED_AMT,

        capp.CA7UNITNUMBER AS UNIT_NO,

        state.TYPECODE AS LIC_JURS_CD,

        capp.LICENSEPLATE AS LIC_PLATE,

        CASE
            WHEN capp.LEASEORRENT = 1 THEN 'Y'
            WHEN capp.LEASEORRENT = 0 THEN 'N'
            ELSE CAST(capp.LEASEORRENT AS VARCHAR)
        END AS LEASE_OR_RENT_FL,

        lenoflease.TYPECODE AS LEN_OF_LEASE_CD,

        capp.CA7CLASSCODE AS CL_CD,

        CASE
            WHEN capp.CA7FLEET = 'Yes' THEN 'Y'
            WHEN capp.CA7FLEET = 'No' THEN 'N'
            ELSE NULL
        END AS FLEET_FL,

        CASE
            WHEN capp.LESSORANEMPLOYEE = 1 THEN 'Y'
            WHEN capp.LESSORANEMPLOYEE = 0 THEN 'N'
            ELSE CAST(capp.LESSORANEMPLOYEE AS VARCHAR)
        END AS LESSOR_AN_EMP_FL,

        capp.CA7OPERATOREXPERIENCE AS PRVT_PSNGR_OPER_EXPER_CD,

        capp.CA7USE AS PRVT_PSNGR_USE_CD,

        capp.CA7TYPE AS PRVT_PSNGR_TYPE_CD,
        ...
        CASE
            WHEN capp.CA7AUTOSFURNISHEDFORREGULARUSE = 'Yes' THEN 'Y'
            WHEN capp.CA7AUTOSFURNISHEDFORREGULARUSE = 'No' THEN 'N'
            ELSE NULL
        END AS REG_USE_FL,

        curr.TYPECODE AS CURR_CD,

        capp.CA7ZIPCODE AS ZIP_CD,

        capp.CA7ZIPCODEOVERRIDE AS ZIP_CD_OVERRIDE,

        capp.CA7TERRITORY AS TERR,

        capp.CA7GARAGINGLOCATION AS GARAGE_LOC,

        capp.CA7PERCENTAGEOFTRANSPORTATIONN AS TRNSP_PCT_TEXT,

        capp.CA7SAFETYSCORE AS SAFETY_SCORE,

        CASE
            WHEN capp.CA7SAFETYSCOREDISCOUNTAPPLIES = 'Yes' THEN 'Y'
            WHEN capp.CA7SAFETYSCOREDISCOUNTAPPLIES = 'No' THEN 'N'
            ELSE NULL
        END AS SAFETY_SCORE_DISC_APPLIES_FL,

        CASE
            WHEN capp.CA7TELEMATICSDISCOUNTAPPLIES = 'Yes' THEN 'Y'
            WHEN capp.CA7TELEMATICSDISCOUNTAPPLIES = 'No' THEN 'N'
            ELSE NULL
        END AS TELEMATICS_DISC_APPLIES_FL,

        capp.CA7USEDASTNCORONDEMAND AS USED_AS_TNC_OR_ON_DEMAND_CD,
    ...
    LEFT JOIN pctl_lengthoflease lenoflease
        ON lenoflease.ID = capp.LENGTHOFLEASE
    LEFT JOIN pctl_state state
        ON state.ID = capp.LICENSESTATE
    LEFT JOIN pctl_currency curr
        ON curr.ID = capp.PREFERREDCOVERAGECURRENCY
    LEFT JOIN has_effective he
        ON he.BRANCHID = capp.BRANCHID AND he.FIXEDID = capp.FIXEDID
    CROSS JOIN etl_control_parm ecp
    WHERE status.TYPECODE = 'Bound'
      AND (
          CAST(COALESCE(capp.EFFECTIVEDATE, polper.PERIODSTART) AS DATE)
          <> CAST(COALESCE(capp.EXPIRATIONDATE, polper.PERIODEND) AS DATE)
