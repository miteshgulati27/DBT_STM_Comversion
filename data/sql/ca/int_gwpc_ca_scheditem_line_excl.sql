...
WITH pcx_ca7lineschedexclitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7lineschedexclitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7commautolineexcl') }}
),
...
SELECT
    ...
    'GWPC'
        ...
        || CAST(polper.PeriodID AS VARCHAR)
        || '-'
        || CAST(parent.CA7CommAutoLine AS VARCHAR)
        AS POL_LINE_KEY,

    NULL AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'CALineExclSI' AS CVRBL_TYPE_KEY,
...
FROM pcx_ca7lineschedexclitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7lineschedexclitem srcx
...
INT_CA_SI_CALINE_EXCLSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_CALINE_EXCLSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
