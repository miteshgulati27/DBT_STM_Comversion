...
WITH pcx_ca7lineschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7lineschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7commautolinecov') }}
),
...
SELECT
    ...
    'GWPC' || '-' || 'CALineCovgSI' AS CVRBL_TYPE_KEY,
...
FROM pcx_ca7lineschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7lineschedcovitem srcx
...
INT_CA_SI_CALINE_COVGSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_CALINE_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
