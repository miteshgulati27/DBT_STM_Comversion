...
WITH pcx_ca7ptschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7ptschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7publictransportcov') }}
),
...
INT_CA_SI_PUBLICTRANSPORT_COVGSI AS (
...
SELECT
    ...
    CASE
        WHEN parent.PublicTransport IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'PublicTransport'
            || '-'
            || CAST(parent.PublicTransport AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'PublicTransportCovgSI' AS CVRBL_TYPE_KEY,
    ...
    'PublicTransportCovgSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7ptschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7ptschedcovitem srcx
...
INT_CA_SI_PUBLICTRANSPORT_COVGSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_PUBLICTRANSPORT_COVGSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_PUBLICTRANSPORT_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
