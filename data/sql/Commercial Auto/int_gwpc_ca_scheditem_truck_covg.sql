...
WITH pcx_ca7trschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7trschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7truckcov') }}
),
...
SELECT
    ...
    CASE
        WHEN parent.Truck IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'Truck'
            || '-'
            || CAST(parent.Truck AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'TruckCovgSI' AS CVRBL_TYPE_KEY,
...
FROM pcx_ca7trschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7trschedcovitem srcx
...
INT_CA_SI_TRUCK_COVGSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_TRUCK_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
