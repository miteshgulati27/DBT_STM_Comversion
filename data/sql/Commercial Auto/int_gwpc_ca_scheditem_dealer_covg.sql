...
WITH pcx_ca7grdschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7grdschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7autodealercov') }}
),
...
SELECT
    ...
    'GWPC' || '-' || 'DealerCovgSI' AS CVRBL_TYPE_KEY,
...
FROM pcx_ca7grdschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7grdschedcovitem srcx
...
INT_CA_SI_DEALER_COVGSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_DEALER_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
