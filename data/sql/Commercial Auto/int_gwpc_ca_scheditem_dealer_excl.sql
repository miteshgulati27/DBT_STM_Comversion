...
WITH pcx_ca7grschedexclitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7grschedexclitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7autodealerexcl') }}
),
...
SELECT
    ...
    CASE
        WHEN parent.AutoDealer IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'Dealer'
            || '-'
            || CAST(parent.AutoDealer AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'DealerExclSI' AS CVRBL_TYPE_KEY,
...
FROM pcx_ca7grschedexclitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7grschedexclitem srcx
...
INT_CA_SI_DEALER_EXCLSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_DEALER_EXCLSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
