...
WITH pcx_ca7grsschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7grsschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7garageservicecov') }}
),
...
INT_CA_SI_GARAGESERVICE_COVGSI AS (
...
SELECT
    ...
    CASE
        WHEN parent.GarageService IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'GarageService'
            || '-'
            || CAST(parent.GarageService AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'GarageServiceCovgSI' AS CVRBL_TYPE_KEY,
    ...
    'GarageServiceCovgSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7grsschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7grsschedcovitem srcx
...
INT_CA_SI_GARAGESERVICE_COVGSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_GARAGESERVICE_COVGSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_GARAGESERVICE_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
