...
WITH pcx_ca7stschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7stschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7specialtypecov') }}
),
...
INT_CA_SI_SPECIALTYPE_COVGSI AS (
...
SELECT
    ...
    CASE
        WHEN parent.SpecialType IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'SpecialType'
            || '-'
            || CAST(parent.SpecialType AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'SpecialTypeCovgSI' AS CVRBL_TYPE_KEY,
    ...
    'SpecialTypeCovgSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7stschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7stschedcovitem srcx
...
INT_CA_SI_SPECIALTYPE_COVGSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_SPECIALTYPE_COVGSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_SPECIALTYPE_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
