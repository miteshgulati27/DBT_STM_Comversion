...
WITH pcx_ca7specialtype AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7specialtype') }}
),
...
INT_CA_VEH AS (
    SELECT DISTINCT
        'GWPC' || '-' || CAST(polper.PERIODID AS VARCHAR) || '-' || 'SpecialType' || '-' || CAST(caspclveh.FIXEDID AS VARCHAR)
            AS CA_CVRBL_KEY,
        ...
        'GWPC' || '-' || CAST(polper.PERIODID AS VARCHAR) || '-' || CAST(caspclveh.LOCATION AS VARCHAR)
            AS GARAGE_LOC_KEY,

        CAST(COALESCE(caspclveh.EFFECTIVEDATE, polper.PERIODSTART) AS DATE) AS END_EFF_DT,

        CAST(COALESCE(caspclveh.EXPIRATIONDATE, polper.PERIODEND) AS DATE) AS END_EXP_DT,

        'GWPC' AS SOURCE_SYSTEM,

        'SpecialType' AS CVRBL_TYPE_CD,

        caspclveh.CA7VIN AS VEH_ID_NO,

        caspclveh.CA7MAKE AS MAKE_NAME,

        caspclveh.CA7MODEL AS MODEL_NAME,

        caspclveh.CA7YEAR AS MODEL_YR,

        caspclveh.CA7VEHICLENUMBER AS VEH_NO,

        caspclveh.CA7ORIGINALCOSTNEW AS ORIG_COST_NEW_AMT,

        caspclveh.CA7STATEDAMOUNT AS STATED_AMT,

        caspclveh.CA7UNITNUMBER AS UNIT_NO,

        state.TYPECODE AS LIC_JURS_CD,

        caspclveh.LICENSEPLATE AS LIC_PLATE,

        CASE
            WHEN caspclveh.LEASEORRENT = 1 THEN 'Y'
            WHEN caspclveh.LEASEORRENT = 0 THEN 'N'
            ELSE CAST(caspclveh.LEASEORRENT AS VARCHAR)
        END AS LEASE_OR_RENT_FL,

        lenoflease.TYPECODE AS LEN_OF_LEASE_CD,

        caspclveh.CA7CLASSCODE AS CL_CD,

        'N' AS FLEET_FL,

        CASE
            WHEN caspclveh.LESSORANEMPLOYEE = 1 THEN 'Y'
            WHEN caspclveh.LESSORANEMPLOYEE = 0 THEN 'N'
            ELSE CAST(caspclveh.LESSORANEMPLOYEE AS VARCHAR)
        ...
        caspclveh.CA7CLAZZ AS SPCL_VEH_CL_CD,

        caspclveh.CA7ENGINESIZE AS ENGINE_SIZE_CD,

        NULL AS GROSS_RECEIPTS_AMT,

        CASE
            WHEN caspclveh.CA7NONOWNEDAUTOS = 'Yes' THEN 'Y'
            WHEN caspclveh.CA7NONOWNEDAUTOS = 'No' THEN 'N'
            ELSE NULL
        END AS NON_OWNED_AUTOS_FL,

        caspclveh.CA7NUMBEROFDAYSVEHICLEISLEASED AS DAYS_VEH_LEASED_CNT,

        caspclveh.CA7NUMBEROFFACTORYTESTINGEMPLS AS FCTRY_TST_EMP_CNT,

        caspclveh.CA7NUMBEROFREPOSSESSEDAUTOS AS REPO_AUTOS_CNT,

        caspclveh.CA7PHYSDAMAGECLASSIFICATION AS PHYS_DMG_CLASS_CD,

        caspclveh.CA7SUPPLEMENTARYTYPE AS SUPP_TYPE_CD,

        caspclveh.CA7TOTALCOSTOFHIRECOLLISION AS COST_HIRE_FOR_COLL_AMT,

        caspclveh.CA7TOTALCOSTOFHIRELIABINSUREDP AS COST_HIRE_FOR_LIAB_AMT,

        caspclveh.CA7TOTALCOSTOFHIRELIABOWNERPRI AS COST_HIRE_FOR_EXCS_LIAB_AMT,

        caspclveh.CA7TOTALCOSTOFHIREOTHTHANCOLLI AS COST_HIRE_FOR_OTC_AMT,

        caspclveh.CA7TYPE AS SPCL_VEH_TYPE_CD,

        NULL AS REG_USE_FL,

        curr.TYPECODE AS CURR_CD,

        caspclveh.CA7ZIPCODE AS ZIP_CD,

        caspclveh.CA7ZIPCODEOVERRIDE AS ZIP_CD_OVERRIDE,

        caspclveh.CA7TERRITORY AS TERR,

        caspclveh.CA7GARAGINGLOCATION AS GARAGE_LOC,
        ...
        caspclveh.CA7VEHICLEINDICATOR AS VEH_INDC_CD

    FROM pcx_ca7specialtype caspclveh
    JOIN pc_policyperiod polper
        ON polper.ID = caspclveh.BRANCHID
    JOIN pc_job job
    ...
    LEFT JOIN pctl_lengthoflease lenoflease
        ON lenoflease.ID = caspclveh.LENGTHOFLEASE
    LEFT JOIN pctl_state state
        ON state.ID = caspclveh.LICENSESTATE
    LEFT JOIN pctl_currency curr
        ON curr.ID = caspclveh.PREFERREDCOVERAGECURRENCY
    LEFT JOIN has_effective he
        ON he.BRANCHID = caspclveh.BRANCHID AND he.FIXEDID = caspclveh.FIXEDID
    CROSS JOIN etl_control_parm ecp
    WHERE status.TYPECODE = 'Bound'
      AND (
          CAST(COALESCE(caspclveh.EFFECTIVEDATE, polper.PERIODSTART) AS DATE)
          <> CAST(COALESCE(caspclveh.EXPIRATIONDATE, polper.PERIODEND) AS DATE)
