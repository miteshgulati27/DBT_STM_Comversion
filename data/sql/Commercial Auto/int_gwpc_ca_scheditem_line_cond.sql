...
WITH pcx_ca7lineschedconditem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7lineschedconditem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7commautolinecond') }}
),
...
SELECT
    ...
    'GWPC' || '-' || 'CALineCondSI' AS CVRBL_TYPE_KEY,
    ...
    CASE
        WHEN src.ScheduleCondition IS NULL THEN NULL
        ELSE 'GWPC' || '-' || CAST(polper.PeriodID AS VARCHAR) || '-' || CAST(src.ScheduleCondition AS VARCHAR)
    END AS CA_COND_KEY,
...
FROM pcx_ca7lineschedconditem src

INNER JOIN parent
    ON parent.FixedID = src.ScheduleCondition
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7lineschedconditem srcx
...
INT_CA_SI_CALINE_CONDSI_cleaned AS (
SELECT
...
{{ m_scd2_scd1(
    'INT_CA_SI_CALINE_CONDSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
