...
WITH pcx_ca7zrschedcovitem AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7zrschedcovitem') }}
),

parent AS (
    SELECT * FROM {{ source('sources_ca', 'pcx_ca7zoneratedcov') }}
),
...
SELECT
    ...
    CASE
        WHEN parent.ZoneRated IS NULL THEN NULL
        ELSE 'GWPC'
            || '-'
            || CAST(polper.PeriodID AS VARCHAR)
            || '-'
            || 'ZoneRated'
            || '-'
            || CAST(parent.ZoneRated AS VARCHAR)
    END AS OWNING_CVRBL_KEY,

    'GWPC' || '-' || 'ZoneRatedCovgSI' AS CVRBL_TYPE_KEY,
    ...
    'ZoneRatedCovgSI' AS CVRBL_TYPE_CD,
...
FROM pcx_ca7zrschedcovitem src
...
WHERE
    ...
    AND (
        ...
        OR NOT EXISTS (
            SELECT 1 FROM pcx_ca7zrschedcovitem srcx
...
INT_CA_SI_ZONERATED_COVGSI_cleaned AS (
SELECT
    ...
    ETL_ROW_EFF_DTS
FROM INT_CA_SI_ZONERATED_COVGSI
)
...
{{ m_scd2_scd1(
    'INT_CA_SI_ZONERATED_COVGSI_cleaned',
    key_cols,
    scd2_cols,
    scd1_cols
) }}
