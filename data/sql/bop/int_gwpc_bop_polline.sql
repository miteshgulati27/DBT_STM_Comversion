...
WITH pc_policyline AS (
    SELECT *
    FROM {{ source('sources_bop', 'pc_policyline') }}
),
...
SELECT
    ...
    NULL AS BLNKT_TYPE_CD,

    src.NumAddInsured AS NUM_ADD_INS,
    ...
    NULL AS SMALL_BUS_TYPE_CD,

    Subtype.TYPECODE
        AS SUBTYPE_CD,

    CAST(NULL AS NUMBER)
        AS VIEW_BUNDLED_COVERAGES_FL,
...
SELECT
    ...
    {{ m_cleanse('VARCHAR_SINGLESPACE', 'BLNKT_TYPE_CD') }}          AS BLNKT_TYPE_CD,
    {{ m_cleanse('INTEGER_ZERO', 'NUM_ADD_INS') }}                   AS NUM_ADD_INS,
    {{ m_cleanse('VARCHAR_SINGLESPACE', 'PAT_CD') }}                 AS PAT_CD,
    {{ m_cleanse('VARCHAR_SINGLESPACE', 'PREFD_COVG_CURR_CD') }}     AS PREFD_COVG_CURR_CD,
    {{ m_cleanse('VARCHAR_SINGLESPACE', 'SMALL_BUS_TYPE_CD') }}      AS SMALL_BUS_TYPE_CD,
    {{ m_cleanse('VARCHAR_SINGLESPACE', 'SUBTYPE_CD') }}             AS SUBTYPE_CD,
    {{ m_cleanse('VARCHAR_BOOLEAN', 'VIEW_BUNDLED_COVERAGES_FL') }}   AS VIEW_BUNDLED_COVERAGES_FL,
    ETL_ROW_EFF_DTS
FROM INT_BOPPOLLINE
